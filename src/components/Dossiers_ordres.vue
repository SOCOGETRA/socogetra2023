<template>
            ...
            <vs-col vs-w="9.5" style="overflow:scroll; height: calc(100vh - 48px);">
              <div style="position: fixed; width: calc(79% - 13px); z-index: 1000;">
                ...
                <div style="width: 730px; float: left; display: inline-block; margin-bottom: -15px; color: black; font-size: 20px;">
                  <span style="color: #C32D39;"><vs-icon icon="home" size="" style="font-size: 18px;"></vs-icon> Administration /</span> <b>IMPORT</b> / Dossier Ordres
                 
                </div>
                <span style="float: right; margin-right: 150px;">
                      <vs-button color="#C32D39"  icon="note_add" style="padding: 5px; position: absolute; width: 150px;" @click="activePrompt2=true">Créer dossier</vs-button>
                  </span>
                    ...
                    <template slot-scope="{data}">
                      <vs-tr :key="indextr" v-for="(tr, indextr) in data" >
                        <vs-td :data="data[indextr].date_doc">
                          <el-link @click="donneurOrdres(data[indextr])" type="primary"><i class="el-icon-document"></i> {{data[indextr].date_doc}}</el-link>
                          
                        </vs-td>

                        <vs-td :data="data[indextr].date_doc">
                            <vs-chip>
                              <b style="font-size: 16px">{{data[indextr].n_dossier}}</b>
                            </vs-chip>
                        </vs-td>
            ...
            <vs-row>
              ...
              <vs-col style="padding: 5px;" vs-justify="center" vs-align="center" vs-w="4">
                ...
                <vs-select
                  ...
                  >
                  <vs-select-item :key="index" :value="item.value" :text="item.value" v-for="item,index in armateur" />
                </vs-select>
              </vs-col>
              <vs-col style="padding: 5px;" vs-justify="center" vs-align="center" vs-w="4">
                <label>N° Dossier</label><br>
                <vs-button color="primary" type="flat" @click="numeroDoc()" v-if="form.n_dossier.length <= 10">Générer un numéro dossier</vs-button>
                <span style="font-size: 18px; font-weight: bold; letter-spacing: 2px" v-else>{{ form.n_dossier }}
                </span>
              </vs-col>
  ...
  export default {
    ...
    methods: {
        ...
        donneurOrdres(a) {
          this.$store.commit('Dossiers', a);
          this.$router.push({ path: '/administration/ordres' });
        },
        modifBon(list) {
          this.donneesC = list;
        ...
        listeOrdres () {
            ...
            fetch('https://socogetra.net/appi_socogetra/soco_dossier_ordres.php?timestamp=' + new Date().getTime())
        ...
        onSubmit() {
              ...
              axios.post('https://socogetra.net/appi_socogetra/soco_dossier_ordres.php', this.form)
              .then((response) => {
                  console.log(response);
                  this.listeOrdres();
