          ...
          <vs-col vs-w="12" style="overflow:scroll; height: calc(100vh - 48px);">
            ...
            <div style="padding: 10px 20px; font-family:'Karla', sans-serif; margin-top: 90px;">
                <el-card class="box-card" style="padding: -15px; border-radius: 10px;">
                  <span><b>{{ donnees.length }} Conteneurs</b> - <el-link type="success" @click="ExportExcel('xls')"><img src="../assets/excel.png" width="14"> Excel</el-link> <el-link v-if="Object.keys(ajoutListe).length !== 0" style="margin-left: 20px" type="success" @click="ExportExcelS('xls')"><img src="../assets/excel.png" width="14"> Selectionnées</el-link></span>
                    ...
                    <vs-tr :state="data[indextr].sejour.length >= 1000?'success':indextr == 2000?'danger':null" :key="indextr" v-for="(tr, indextr) in data">
                      <vs-td :data="data[indextr].id_doc">
                        <span style="color: blue; font-size: 12px; text-align: center; cursor: pointer" >
                          <vs-icon icon="radio_button_checked" size="small" color="green" @click="SelectChoix(data[indextr])" v-if="data[indextr].sejour == 'ok'"></vs-icon>
                          <vs-icon icon="check_box_outline_blank" size="small" color="gray" @click="SelectChoix(data[indextr])" v-if="data[indextr].sejour != 'ok'"></vs-icon>
                        </span>
                        </vs-td>
                      ...
                      <vs-td style="font-size: 12px; background: #409eff2b" :data="data[indextr].n_doc">
                        <span style="color:red; font-weight: bold; text-align: center" >{{data[indextr].n_doc}}</span>
                      </vs-td>

                      <vs-td style="font-size: 12px" :data="data[indextr].n_conteneur">
                        <b style="color: blue">{{data[indextr].n_conteneur}}</b>
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].type_c">
                        {{data[indextr].type_c}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].nbre_tcs">
                        {{data[indextr].nbre_tcs}}
                      </vs-td>

                      <vs-td style="font-size: 12px; color: red; text-align: center" :data="data[indextr].n_declaration">
                        {{data[indextr].n_declaration}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].bl">
                        {{data[indextr].bl}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].statut">
                      ...
                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].armateur">
                        {{data[indextr].armateur}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].client">
                        {{data[indextr].client}}
                      </vs-td>
                      
                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].camion">
                        {{data[indextr].camion}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].chauffeur">
                        {{data[indextr].chauffeur}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].zone_livraison">
                        {{data[indextr].zone_livraison}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].zone_facturation">
                        {{data[indextr].zone_facturation}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].badt">
                        {{data[indextr].badt}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].do">
                        {{data[indextr].do}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].sortie_port">
                        {{data[indextr].sortie_port}}
                      </vs-td>

                      <vs-td style="font-size: 12px; text-align: center" :data="data[indextr].date_recup">
                        {{data[indextr].date_recup}}
                      </vs-td>

                      
                      <vs-td style="font-size: 10px" :data="data[indextr].id_doc">
                        <el-button-group style="display: flex;">
                          <el-button style=" padding: 5px" type="primary" icon="el-icon-edit" @click="modifBon(data[indextr])"></el-button>
                          <el-button style=" padding: 5px" type="danger" icon="el-icon-delete" @click="supBon(data[indextr].id_doc, data[indextr].n_conteneur)"></el-button>
                        </el-button-group>
                      </vs-td>
                    </vs-tr>
                  </template>
       ...
       <vs-popup classContent="popup-example"  title="Modification Ordre" :active.sync="popupActivo5">
          <div>
            ...
            <vs-row>
              ...
              <vs-col style="padding: 5px; background: rgb(170, 170, 224)" vs-justify="center" vs-align="center" vs-w="4">
                <label>REQUETE</label>
                <el-input type="date" v-model="form.date_requete" placeholder="Date de Requette">
                </el-input>
              </vs-col>
            </vs-row>
            <vs-row>
  ...
  methods: {
    tableRowClassName({row, rowIndex}) {
      ...
      SelectChoix (ajout) {
        ...
        const itemToUpdate = this.donnees.findIndex(item => item.id_doc === ajout.id_doc);
        if (itemToUpdate !== -1) {
          // Mettez à jour le champ "age" de l'objet avec la nouvelle valeur
          this.donnees[itemToUpdate].sejour = 'ok';
        }
      ...
      listeDossier () {
          ...
          axios.get('https://socogetra.net/appi_socogetra/soco_ordre_fichier.php?timestamp=' + new Date().getTime())
          .then((response) => {
      ...
      listeZones () {
        ...
        onDelete() {
              axios.delete('https://socogetra.net/appi_socogetra/soco_ordre_fichier.php?id='+this.deletebon)
              .then((response) => {
