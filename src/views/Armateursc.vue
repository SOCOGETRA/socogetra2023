            ...
            <vs-col vs-w="12" style="overflow:scroll; height: calc(100vh - 48px);">
              ...
              <div style="padding: 10px 20px; font-family:'Karla', sans-serif; margin-top: 90px;">
                  <el-card class="box-card" style="padding: -15px; border-radius: 10px;">
                    <span><b>{{ donnees.length }} Conteneurs</b> - <el-link type="success" @click="ExportExcel('xls')"><img src="../assets/excel.png" width="14"> Importer Livraisons</el-link> / <el-link type="success" @click="ExportExcells('xls')"><img src="../assets/excel.png" width="14"> POINT IMPORT</el-link></span>
                  ...
                  <vs-divider/>
                  <el-card class="box-card" style="padding: -15px; border-radius: 10px;">
                    <span><b>{{ donneesRecup.length }} Conteneurs</b> - <el-link type="success" @click="ExportExcell('xls')"><img src="../assets/excel.png" width="14"> Importer Récupération</el-link> / <el-link type="success" @click="ExportExcellr('xls')"><img src="../assets/excel.png" width="14"> POINT EXPORT</el-link></span>
                    <table border="1" ref="exportable_tablee" v-show="false">
                      ...
                      <tr v-for="(item, i) in donneesRecup" :key="i">
                        <td>{{item.date_recept}}</td>
                          <td>{{item.n_conteneur}}</td>
                          <td>{{item.bl}}</td>
                          <td>{{item.type_c}}</td>
                    ...
                    <table border="1" ref="exportable_tablees" v-show="false">
                      <tbody >
                      <tr>
                          <th>ARMATEUR</th>
                          <th>ZONE</th>
                      ...
                      <tr v-for="(item, i) in donneesPoint" :key="i">
                        <td>{{item.armateur}}</td>
                          <td>{{item.zone_facturation}}</td>
                          <td>{{item.mode}}</td>
                          <td>{{item.type_c}}</td>
                          <td>{{item.nombre_de_conteneurs}}</td>
                          <td>{{item.prix}}</td>
                          <td>{{parseInt(item.prix) * parseInt(item.nombre_de_conteneurs)}}</td>
                          <td>{{(parseInt(item.prix) * parseInt(item.nombre_de_conteneurs))*0.4}}</td>
                      </tr>
                      </tbody>
                    </table>
                    <table border="1" ref="exportable_tableesr" v-show="false">
                      <tbody >
                      <tr>
                          <th>ARMATEUR</th>
                          <th>ZONE</th>
                      ...
                      <tr v-for="(item, i) in donneesPointr" :key="i">
                        <td>{{item.armateur}}</td>
                          <td>{{item.zone_facturation}}</td>
                          <td>{{item.mode}}</td>
                          <td>{{item.type_c}}</td>
                          ...
                          <td>{{item.prix}}</td>
                          <td>{{parseInt(item.prix) * parseInt(item.nombre_de_conteneurs)}}</td>
                          <td>{{(parseInt(item.prix) * parseInt(item.nombre_de_conteneurs))*0.6}}</td>
                      </tr>
                      </tbody>
                    </table>
                      <vs-table search max-items="20" pagination  :data="donneesRecup" style="font-size: 12px">
                      <template slot="header">
                        <h3>
                          RECUPERATIONS
                        </h3>
    ...
    methods: {
      tableRowClassName({row, rowIndex}) {
        ...
        ordre () {
          ...
          ExportExcell(type, fn, dl) {
            let name="RECUPERATION";
          ...
          ExportExcells(type, fn, dl) {
            let name="POINTS IMPORT";
            var elt = this.$refs.exportable_tablees;
            var wb = XLSX.utils.table_to_book(elt, {sheet:"Sheet JS"});
            return dl ?
              XLSX.write(wb, {bookType:type, bookSST:true, type: 'base64'}) :
            XLSX.writeFile(wb, fn || ((name + '.'|| 'SheetJSTableExport.') + (type || 'xlsx')));
          },
          ExportExcellr(type, fn, dl) {
            let name="POINTS EXPORT";
            var elt = this.$refs.exportable_tableesr;
            var wb = XLSX.utils.table_to_book(elt, {sheet:"Sheet JS"});
            return dl ?
              XLSX.write(wb, {bookType:type, bookSST:true, type: 'base64'}) :
            XLSX.writeFile(wb, fn || ((name + '.'|| 'SheetJSTableExport.') + (type || 'xlsx')));
        ...
        listeDossier () {
            ...
            axios.get('https://socogetra.net/appi_socogetra/soco_ordre_armateurc.php?date_db=' + this.$route.params.debut + '&date_fn=' + this.$route.params.fin + '&timestamp=' + new Date().getTime())
            .then((response) => {
              this.donnees = response.data.data;
              this.donneesRecup = response.data.recup;
              this.donneesPoint = response.data.datanb;
              this.donneesPointr = response.data.recupnb;
              this.$vs.loading.close()
            }, (error) => {
              console.log(error);
              this.$vs.loading.close()
